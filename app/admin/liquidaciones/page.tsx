...
export default function LiquidacionesPage() {
  ...
  return (
    <div>
        ...
        {metricas.map((m) => (
          <div key={m.label} className="rounded-xl border border-[#e8e8e8] p-5">
            <div className="flex items-start justify-between">
              <div>
                <p style={{ fontFamily: "var(--font-body)", fontSize: "12px", color: "#666666", textTransform: "uppercase", letterSpacing: "0.05em" }}>{m.label}</p>
                <p className="mt-1" style={{ fontFamily: "var(--font-body)", fontSize: "24px", fontWeight: "700", color: "#111111" }}>{m.valor}</p>
              </div>
      ...
      {/* Pendientes */}
      <h2 className="mb-4" style={{ fontFamily: "var(--font-titulo)", fontSize: "20px", color: "#111111" }}>
        Pendientes de pago
      </h2>
      {pendientes.length === 0 ? (
        <div className="rounded-xl border border-[#e8e8e8] p-10 text-center mb-10" style={{ backgroundColor: "#f8f7f5" }}>
          <p style={{ fontFamily: "var(--font-body)", color: "#666666" }}>Sin artistas pendientes de liquidación.</p>
        </div>
      ) : (
            ...
            return (
              <div key={a.id} className="flex items-center gap-4 rounded-xl border border-[#e8e8e8] p-4">
                {a.foto_url ? (
                  ...
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shrink-0" style={{ backgroundColor: "#e8003d", fontFamily: "var(--font-body)" }}>
                    {a.nombre[0]?.toUpperCase()}
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <p style={{ fontFamily: "var(--font-body)", fontWeight: 600, color: "#111111" }}>{a.nombre}</p>
                  {tieneBanco ? (
                    <p className="text-xs" style={{ fontFamily: "var(--font-body)", color: "#666666" }}>
                      {a.banco} · {a.tipo_cuenta} · {a.cuenta_bancaria}
                    </p>
                  ) : (
                    <span className="text-xs px-2 py-0.5 rounded" style={{ fontFamily: "var(--font-body)", backgroundColor: "#fee2e2", color: "#991b1b" }}>
                      Sin datos bancarios
                    </span>
                  )}
                </div>
                <p style={{ fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "18px", color: "#e8003d", marginRight: "12px" }}>
                  {formatCLP(a.monto)}
                </p>
                <button
                  onClick={() => abrirPago(a)}
      ...
      <div className="rounded-xl border border-[#e8e8e8] overflow-hidden">
        {historial.length === 0 ? (
          <div className="p-8 text-center">
            <p style={{ fontFamily: "var(--font-body)", color: "#666666" }}>Sin pagos registrados.</p>
          </div>
        ) : (
            ...
            <tbody>
              {historial.map((l, i) => (
                <tr key={l.id} style={{ borderTop: i > 0 ? "1px solid #f0f0f0" : undefined }}>
                  ...
                  <td className="px-4 py-3" style={{ fontFamily: "var(--font-body)", fontWeight: 700, color: "#e8003d" }}>
                    {formatCLP(l.monto)}
                  </td>
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-4 py-2">
            <div>
              ...
              <p style={{ fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "22px", color: "#e8003d" }}>
            ...
            <div>
              ...
              <input value={comprobante} onChange={(e) => setComprobante(e.target.value)} className={inputClass} style={{ fontFamily: "var(--font-body)" }} placeholder="Ej: 123456789" />
            </div>
            <div>
              <label style={{ fontFamily: "var(--font-body)", fontSize: "13px", color: "#444444", display: "block", marginBottom: "5px" }}>
                Fecha del pago
              </label>
              <input type="date" value={fechaPago} onChange={(e) => setFechaPago(e.target.value)} className={inputClass} style={{ fontFamily: "var(--font-body)" }} />
            </div>
            <div>
              <label style={{ fontFamily: "var(--font-body)", fontSize: "13px", color: "#444444", display: "block", marginBottom: "5px" }}>
              ...
              <label className="flex items-center gap-2 cursor-pointer border border-dashed border-[#e8e8e8] rounded-lg px-4 py-3 hover:border-[#e8003d] transition-colors">
                ...
                <span style={{ fontFamily: "var(--font-body)", fontSize: "13px", color: "#666666" }}>
          ...
          <DialogFooter>
            <button onClick={() => setPagarOpen(false)} className="px-4 py-2 rounded border text-sm" style={{ fontFamily: "var(--font-body)", borderColor: "#e8e8e8", color: "#666666" }}>
              Cancelar
            </button>
            <button
              ...
              style={{ fontFamily: "var(--font-body)", backgroundColor: pagando ? "#f0a0b0" : "#e8003d" }}
            >
