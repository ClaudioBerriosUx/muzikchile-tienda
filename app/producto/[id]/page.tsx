...
function StockIndicator({ stock }: { stock: number }) {
  if (stock === 0) {
    return (
      <span style={{ fontFamily: "Barlow, sans-serif", fontSize: "14px", color: "#e8003d" }}>
        ● Sin stock
      </span>
    );
  }
  if (stock <= 5) {
    return (
      <span style={{ fontFamily: "Barlow, sans-serif", fontSize: "14px", color: "#f59e0b" }}>
        ● Últimas {stock} unidades
      </span>
    );
  }
  return (
    <span style={{ fontFamily: "Barlow, sans-serif", fontSize: "14px", color: "#22c55e" }}>
      ● {stock} disponibles
    </span>
  );
}
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-sm" style={{ fontFamily: "Barlow, sans-serif", color: "#999999" }}>
              ...
              {categoria && (
                <>
                  <span>/</span>
                  <Link href={`/tienda?categoria=${categoria.id}`} className="hover:text-[#e8003d] transition-colors">
                    {categoria.nombre}
                  </Link>
                </>
              )}
            ...
            {/* Selector cantidad (solo físico con stock) */}
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (!sinStock) e.currentTarget.style.backgroundColor = "#c5002e";
              }}
              onMouseLeave={(e) => {
                if (!sinStock) e.currentTarget.style.backgroundColor = "#e8003d";
              }}
            >
              {sinStock ? "Sin stock" : "Agregar al carrito"}
            </button>

            {/* Botón ver tienda artista */}
            {artista && (
              <Link
                href={`/artista/${artista.slug}`}
